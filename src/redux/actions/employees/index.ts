...
export const getEmployees = () => (dispatch: Dispatch) => {
  fetchEmployees().then((response) => {
    dispatch({
      type: ACTIONTYPES.EMPLOYEES.SET_EMPLOYEES,
...
export const getEmployeeById = (id: string) => (dispatch: Dispatch) => {
  fetchEmployeeById(id).then((response) => {
    dispatch({
      type: ACTIONTYPES.EMPLOYEES.SET_EMPLOYEE_FOCUS,
      payload: response.data,
    });
  });
};

export const setFocusedEmployee =
  (employee: Employee) => (dispatch: Dispatch) => {
    dispatch({
      type: ACTIONTYPES.EMPLOYEES.SET_EMPLOYEE_FOCUS,
      payload: employee,
    });
